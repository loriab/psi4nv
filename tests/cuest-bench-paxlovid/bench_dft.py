"""
Paxlovid DFT Benchmark: cuEST vs DF
====================================
Tests B3LYP/STO-3G on Paxlovid (67 atoms) with both SCF_TYPE DF (CPU)
and SCF_TYPE CUEST (GPU), verifying energies match.

Usage:
    conda run -n p4dev python bench_dft.py [--nthreads N]
"""
...
parser = argparse.ArgumentParser(description="Paxlovid cuEST DFT benchmark")
...
psi4.core.set_output_file("bench_dft.out", False)
...
print("=" * 72)
print("Paxlovid DFT Benchmark: cuEST (GPU) vs DF (CPU)")
...
for func in functionals:
    print(f"--- Functional: {func.upper()} / STO-3G ---")
    ...
    for scf_type in scf_types:
        ...
        energy = psi4.energy(func)
        t1 = time.perf_counter()

        wall = t1 - t0
        niter = int(psi4.variable("SCF ITERATIONS"))

        results[(func, scf_type)] = {
            "energy": energy,
            "wall": wall,
            "niter": niter,
        }
    ...
    e_df = results[(func, "DF")]["energy"]
    e_cu = results[(func, "CUEST")]["energy"]
    diff = abs(e_df - e_cu)
...
print("=" * 72)
for func in functionals:
    w_df = results[(func, "DF")]["wall"]
    w_cu = results[(func, "CUEST")]["wall"]
    speedup = w_df / w_cu if w_cu > 0 else float("inf")
    print(f"  {func.upper():<12}  DF({nthreads}t): {w_df:8.2f}s   cuEST(GPU): {w_cu:8.2f}s   speedup: {speedup:.2f}x")
print()
print("Done.")
