"""
Paxlovid (Nirmatrelvir) RHF Benchmark: cuEST vs DF
====================================================
67-atom molecule (C23 H32 F3 N5 O4), neutral singlet.

Runs RHF/STO-3G and RHF/def2-SVP with both SCF_TYPE DF (CPU, all cores)
and SCF_TYPE CUEST (GPU), comparing wall-clock times and verifying
energies match.

Usage:
    conda run -n p4dev env PYTHONPATH=/path/to/objdir/stage/lib \
        python bench_paxlovid.py [--nthreads N]

The cuEST reference energies from the cuEST standalone test suite:
...
parser = argparse.ArgumentParser(description="Paxlovid cuEST vs DF benchmark")
...
psi4.core.set_output_file("bench_paxlovid.out", False)

# Paxlovid (Nirmatrelvir) from cuEST examples
mol = psi4.geometry("""
    symmetry c1
    0 1
...
cuest_standalone_refs = {
    "sto-3g":   -1737.1712774897262,
    "def2-svp": -1758.3044466471508,
}
...
benchmarks = [
    {"basis": "sto-3g",  "label": "STO-3G"},
    {"basis": "def2-svp", "label": "def2-SVP"},
]

scf_types = ["DF", "CUEST"]

print("=" * 72)
print("Paxlovid (Nirmatrelvir) RHF Benchmark: cuEST (GPU) vs DF (CPU)")
...
for bm in benchmarks:
    ...
    print(f"--- Basis: {label} ---")
    print(f"{'SCF_TYPE':<10} {'Energy (Eh)':<24} {'Wall Time (s)':<16} {'Iterations'}")
    print("-" * 66)
    ...
    for scf_type in scf_types:
        ...
        results[(basis, scf_type)] = {
            "energy": energy,
            "wall": wall,
            "niter": niter,
        }

        print(f"{scf_type:<10} {energy:<24.12f} {wall:<16.2f} {niter}")

    # Energy agreement check
    e_df = results[(basis, "DF")]["energy"]
    e_cu = results[(basis, "CUEST")]["energy"]
    diff = abs(e_df - e_cu)
    match = "PASS" if diff < 1e-6 else "FAIL"
    print(f"  DF vs cuEST delta: {diff:.2e}  [{match}]")

    # Check against cuEST standalone reference if available
    if basis in cuest_standalone_refs:
        ref = cuest_standalone_refs[basis]
        diff_ref = abs(e_cu - ref)
        match_ref = "PASS" if diff_ref < 1e-5 else "FAIL"
        print(f"  cuEST vs standalone ref delta: {diff_ref:.2e}  [{match_ref}]")
...
for bm in benchmarks:
    basis = bm["basis"]
    label = bm["label"]
    w_df = results[(basis, "DF")]["wall"]
    w_cu = results[(basis, "CUEST")]["wall"]
    speedup = w_df / w_cu if w_cu > 0 else float("inf")
    print(f"  {label:<12}  DF({nthreads}t): {w_df:8.2f}s   cuEST(GPU): {w_cu:8.2f}s   speedup: {speedup:.2f}x")
print()
print("Done.")
